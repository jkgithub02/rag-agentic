...
def agent_step_planning_prompt(
    *,
    ...
    return (
        ...
        "- web_search: use when (1) local evidence quality is weak (< 0.5), (2) comparative queries mention entities not found in local docs (e.g., 'X and Y' when X or Y is not in docs), (3) queries about current events/latest research, or (4) after 2-3 search_documents iterations with high quality but suspicion that key entities are missing. Examples: 'latest research on X', 'current state of Y', 'compare A and B' (when B might not be in docs). Web results are labeled separately from local evidence.\n"
        "- finalize: use when evidence appears sufficient across all subqueries to proceed to validation/generation.\n"
        "IMPORTANT: For comparative questions (e.g., 'compare X and Y'), if you've searched local docs 2-3 times with high quality scores but suspect entity Y is missing from the knowledge base, try web_search to fill the gap before finalizing.\n"
